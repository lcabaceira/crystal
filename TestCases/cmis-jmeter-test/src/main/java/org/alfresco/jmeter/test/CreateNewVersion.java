...
public class CreateNewVersion implements JavaSamplerClient{
	...
	public SampleResult runTest(JavaSamplerContext runTestContext) {
		...
		try{
	        ...
	        Document privateWorkingCopy = (Document) cmis.checkOut(document);
	        
	        ContentStream updatedContentStream = cmis.appendToContentStream(privateWorkingCopy.getContentStream(), document.getName(), "plain/text");
	        
	        try {
	            privateWorkingCopy.checkIn(true, null, updatedContentStream, "Added major version with new content.");
	            
	            result.sampleEnd();
		        result.setSuccessful(true);
		        result.setResponseMessage("Successfully created new version.");
		        result.setResponseCodeOK();
	        } catch (CmisBaseException e) {
	            privateWorkingCopy.cancelCheckOut();
	            
	            result.sampleEnd();
	            result.setSuccessful(false);
	            result.setResponseMessage("Failed to create new version: " + e);
