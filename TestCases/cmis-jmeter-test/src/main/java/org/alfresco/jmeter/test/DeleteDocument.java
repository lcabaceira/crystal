...
public class DeleteDocument implements JavaSamplerClient{
	...
	public SampleResult runTest(JavaSamplerContext runTestContext) {
		...
		try{
	        ...
	        long total = parentFolder.getChildren().getTotalNumItems();
	        logger.info("Successfully retrieved children with count: " + total);
	        
	        Document document = (Document) cmis.getFirstChild(parentFolder);
	        String documentName = document.getName();
	        cmis.getCmisSession().delete(document);
	        
	        result.sampleEnd();
	        result.setSuccessful(true);
	        result.setResponseMessage("Successfully deleted document with name: " + documentName);
	        result.setResponseCodeOK();
		}
		catch(Exception e){
			result.sampleEnd();
            result.setSuccessful(false);
            result.setResponseMessage("Failed to delete document: " + e);
